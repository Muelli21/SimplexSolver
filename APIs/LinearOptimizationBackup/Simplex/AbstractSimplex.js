...
class AbstractSimplex {
    ...
    checkCycling() {
        ...
        if (this.tableau.getTableauStates().has(TableauState.CYCLING)) { return; }
        ...
        console.log("Cycling check 2!");

        let previousBases = this.tableau.getArchivedInformation()[0];
        let basis = this.tableau.getBasis();

        for (let previousBasis of previousBases) {
            if (!math.equal(previousBasis, basis).includes(false)) {
                console.log(basisHash(basis));
                console.log(basisHash(previousBasis));

                console.log(basis);
                console.log(previousBasis);

                this.tableau.addTableauState(TableauState.CYCLING);
                console.log("The tableau is cycling! From now on, bland's rule is applied!");
                return;
            }
        }
    }
...
basisHash = array => array.reduce((prev, current) => ((prev << 5) - prev) + current);
