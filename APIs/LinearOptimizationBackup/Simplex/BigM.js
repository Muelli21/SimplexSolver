...
class BigM extends AbstractSimplex {
    ...
    findPivotElement() {
        ...
        for (let index = 1; index < bigMMatrix.size()[1]; index++) {
            ...
            if (objectiveFunctionCoefficient > highestNumber) {
                ...
                console.log("Update in highest number: ")
                console.log(objectiveFunctionCoefficient + " > " + highestNumber);

                highestNumber = objectiveFunctionCoefficient;
                columnIndex = index;

                console.log("BigM was relevant");

                if (this.tableau.getTableauStates().has(TableauState.CYCLING)) {
                    console.log("Stopped search for column because of bland's rule!")
                    break;
                }
            }
        }
        ...
        if (highestNumber == 0) {
            for (let index = 1; index < matrix.size()[1]; index++) {
                ...
                if (objectiveFunctionCoefficient > highestNumber) {
                    ...
                    console.log("BigM was not relevant");
        ...
        for (let currentRowIndex = 0; currentRowIndex < matrix.size()[0] - 1; currentRowIndex++) {
            ...
            if (this.tableau.getTableauStates().has(TableauState.CYCLING)) {
                if (fraction >= 0 && fraction == smallestFraction && basis[currentRowIndex] < basis[lastRowIndex]) {
                    console.log("Bland's rule!");
    ...
    checkFeasibility() {
        ...
        if (math.sum(this.tableau.getObjectiveFunctionBigMCoefficients()) == 0) {
